// components/Home/Home.jsx
import React, { useContext, useEffect, useState } from 'react';
import Header from '../../components/Header/Header';
import Footer from '../../components/Footer/Footer';
import './Home.less';
import uf from '../../assets/uf_logo.png';
import { useTranslator } from '../../Utils/useTranslator'; // Ensure this is the correct path
import { LanguageContext } from '../../Utils/LanguageContext'; // Import LanguageContext

function Home({textScale, textScaleUpdate}) {
  const { language } = useContext(LanguageContext); // Get current language
  const translate = useTranslator(); // Get the translate function
  
  // State to hold translated texts
  const [translatedTexts, setTranslatedTexts] = useState({
    welcomeTitle: '',
    introText: '',
    startFreeTrialButton: '',
    learnMoreButton: '',
    featuresTitle: '',
    blockBasedCodingTitle: '',
    blockBasedCodingDescription: '',
    interactiveTutorialsTitle: '',
    interactiveTutorialsDescription: '',
    realWorldProjectsTitle: '',
    realWorldProjectsDescription: '',
    educationalBenefitsTitle: '',
    benefitsIntro: '',
    criticalThinkingBenefit: '',
    creativityBenefit: '',
    collaborationBenefit: '',
    communicationBenefit: '',
    testimonialsTitle: '',
    testimonial1: '',
    testimonial2: '',
    callToActionTitle: '',
    joinNowButton: '',
  });

  // Function to load translations
  const loadTranslations = async () => {
    const welcomeTitle = await translate('Welcome to CodeSparks - Where Fun Meets Function in Coding!');
    const introText = await translate('Step into a world where coding comes alive with colors, sounds, and interactive challenges! CodeSparks is the perfect place for young minds to learn the essentials of programming with Arduino-based block coding. With a mix of storytelling, puzzles, and hands-on projects, we make learning to code an adventure that captures the imagination and sparks a lifelong passion for technology.');
    const startFreeTrialButton = await translate('Start Your Free Trial');
    const learnMoreButton = await translate('Learn More');
    const featuresTitle = await translate('Amazing Features to Boost Your Coding Skills!');
    const blockBasedCodingTitle = await translate('Drag & Drop Coding Blocks');
    const blockBasedCodingDescription = await translate('Build your first program with easy-to-understand coding blocks! Our colorful interface is designed to teach coding logic in a way that feels like play.');
    const interactiveTutorialsTitle = await translate('Interactive Tutorials');
    const interactiveTutorialsDescription = await translate('Get guided through your first coding journey with fun tutorials that feel like a game. Each lesson is a step towards becoming a junior coding wizard!');
    const realWorldProjectsTitle = await translate('Real-World Projects');
    const realWorldProjectsDescription = await translate('Connect code to the real world by controlling lights, motors, and sensors with your creations. Watch your code come to life in exciting ways!');
    const educationalBenefitsTitle = await translate('Educational Benefits Galore!');
    const benefitsIntro = await translate('At CodeSparks, we believe in learning that grows with you. Our platform offers a range of benefits designed to nurture young minds:');
    const criticalThinkingBenefit = await translate('Critical Thinking - Solve problems and design projects with a logical approach.');
    const creativityBenefit = await translate('Creativity - Let your imagination soar with unlimited project possibilities.');
    const collaborationBenefit = await translate('Collaboration - Team up with friends and tackle coding challenges together.');
    const communicationBenefit = await translate('Communication - Share your projects and ideas with the CodeSparks community.');
    const testimonialsTitle = await translate('Hear From Our Happy Coders!');
    const testimonial1 = await translate('"CodeSparks has made coding so much fun for my kids! They can\'t wait to show me what they\'ve built after each lesson!" - Happy Parent');
    const testimonial2 = await translate('"I love how the lessons make me think. I feel like a real programmer!" - Excited Student');
    const callToActionTitle = await translate('Ready to Start Your Coding Adventure?');
    const joinNowButton = await translate('Join Now!');


    setTranslatedTexts({ 
    welcomeTitle,
    introText,
    startFreeTrialButton,
    learnMoreButton,
    featuresTitle,
    blockBasedCodingTitle,
    blockBasedCodingDescription,
    interactiveTutorialsTitle,
    interactiveTutorialsDescription,
    realWorldProjectsTitle,
    realWorldProjectsDescription,
    educationalBenefitsTitle,
    benefitsIntro,
    criticalThinkingBenefit,
    creativityBenefit,
    collaborationBenefit,
    communicationBenefit,
    testimonialsTitle,
    testimonial1,
    testimonial2,
    callToActionTitle,
    joinNowButton,
    });
  };

  // Load translations when component mounts or language changes
  useEffect(() => {
    loadTranslations();
  }, [language]);
  
    return (
  <div className="home-container">
    <Header textScale = {textScale} textScaleUpdate = {textScaleUpdate}/>
    <main className="home-main-content">
      {/* Introduction Section */}
<section className="home-introduction" style={{ textAlign: 'center', marginBottom: '20px' }}>
  <h1 className="intro-title" style={{ fontSize: `${25 * textScale}px`, margin: '10px 0' , fontWeight: 'bold' }}>{translatedTexts.welcomeTitle}</h1>
  <p className="intro-text" style={{ fontSize: `${20 * textScale}px`}}>{translatedTexts.introText}</p>
  <div className="intro-call-to-action" style={{ margin: '20px 0', color: 'white' }}>
    <button className="start-free-trial" style={{ marginRight: '10px' }}>{translatedTexts.startFreeTrialButton}</button>
    <button className="learn-more">{translatedTexts.learnMoreButton}</button>
  </div>
</section>


      {/* Features Section */}
<section className="home-features">
  <h2 className="features-title" style={{ fontSize: `${25 * textScale}px` }}>{translatedTexts.featuresTitle}</h2>
  <div className="features-grid flex-container">
    {/* Feature 1: Block-Based Coding */}
    <div className="feature-item">
      <img src='https://atlassianblog.wpengine.com/wp-content/uploads/2020/12/brand-5328-t2-blog-illustration-17-ways-to-get-kids-coding-1120x545-@2x-1560x760.png' alt="Colorful Block-Based Coding" className="feature-image" />
      <h3 className="feature-title" style={{ fontSize: `${20 * textScale}px`, fontWeight: 'bold' }}>
      {translatedTexts.blockBasedCodingTitle}
</h3>

      <p className="feature-description" style={{ fontSize: `${20 * textScale}px`}}>{translatedTexts.blockBasedCodingDescription}</p>
    </div>
    {/* Feature 2: Interactive Tutorials */}
    <div className="feature-item">
      <img src="data:image/jpeg;base64," alt="Interactive Tutorials" className="feature-image" />
      <h3 className="feature-title" style={{ fontSize: `${20 * textScale}px`, fontWeight: 'bold' }}>{translatedTexts.interactiveTutorialsTitle}</h3>
      <p className="feature-description" style={{ fontSize: `${20 * textScale}px`}}>{translatedTexts.interactiveTutorialsDescription}</p>
    </div>
    {/* Feature 3: Hardware Integration */}
    <div className="feature-item">
      <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRGtDFMQ7RIi14YPMBRure1ldyVg_XrOZVSMw&usqp=CAU" alt="Arduino Hardware Projects" className="feature-image" />
      <h3 className="feature-title" style={{ fontSize: `${20 * textScale}px`, fontWeight: 'bold' }}>{translatedTexts.realWorldProjectsTitle}</h3>
      <p className="feature-description" style={{ fontSize: `${20 * textScale}px`}}>{translatedTexts.realWorldProjectsDescription}</p>
    </div>
  </div>
</section>


      {/* Educational Benefits Section */}
      <section className="home-educational-benefits">
        <h2 className="benefits-title" style={{ fontSize: `${25 * textScale}px` , fontWeight: 'bold' }}>{translatedTexts.educationalBenefitsTitle}</h2>
        <p className="benefits-intro" style={{ fontSize: `${20 * textScale}px`}}>{translatedTexts.benefitsIntro}</p>
        <ul className="benefits-list">
          <li className="benefit-item" style={{ fontSize: `${20 * textScale}px`}}>
  <span style={{ color: 'blue' }}>{translatedTexts.criticalThinkingBenefit}</span></li>

          <li className="benefit-item" style={{ fontSize: `${20 * textScale}px`}}>
  <span style={{ color: 'blue' }}>{translatedTexts.creativityBenefit}</span> </li>

          <li className="benefit-item" style={{ fontSize: `${20 * textScale}px`}}>
  <span style={{ color: 'blue' }}>{translatedTexts.collaborationBenefit}</span> </li>

<li className="benefit-item" style={{ fontSize: `${20 * textScale}px`}}>
  <span style={{ color: 'blue' }}>{translatedTexts.communicationBenefit}</span></li>

        </ul>
      </section>

      {/* Testimonials Section */}
      <section className="home-testimonials">
        <h2 className="testimonials-title" style={{ fontSize: `${25 * textScale}px`, fontWeight: 'bold' }}>{translatedTexts.testimonialsTitle}</h2>
        <div className="testimonials-container">
          {/* Testimonial 1 */}
          <blockquote className="testimonial">
            <p style={{ fontSize: `${20 * textScale}px`}}>{translatedTexts.testimonial1}</p>
          </blockquote>
          {/* Testimonial 2 */}
          <blockquote className="testimonial">
            <p style={{ fontSize: `${20 * textScale}px`}}>{translatedTexts.testimonial2}</p>
          </blockquote>
        </div>
      </section>

      {/* Call to Action Section */}
      <section className="home-call-to-action">
        <h2 className="call-to-action-title" style={{ fontSize: `${22 * textScale}px` }}>{translatedTexts.callToActionTitle}</h2>
        <button className="join-now-button">{translatedTexts.joinNowButton}</button>
      </section>
    </main>
    <Footer textScale = {textScale} textScaleUpdate = {textScaleUpdate}/>
  </div>
  );
};

export default Home;
